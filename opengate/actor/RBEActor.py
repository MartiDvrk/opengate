...
class RBEActor(g4.GateRBEActor, gate.ActorBase):
    """
    RBEActor: compute a 3D edep/dose map for deposited
    ...
    Options
        - RBEd only for the moment
        - later: RBEt, Q, fluence ...
    ...
    def __str__(self):
        u = self.user_info
        s = f'RBEActor "{u.name}": dim={u.size} spacing={u.spacing} {u.output} tr={u.translation}'
        return s
    ...
    def StartSimulationAction(self):
        ...
        try:
            ...
            gate.fatal(f"Error in the RBEActor {self.user_info.name}")
        ...
        if not self.first_run:
            gate.warning(f"Not implemented yet: RBEActor with several runs")
        ...
        if vol_type == "Image":
            ...
            if self.user_info.img_coord_system:
                gate.warning(
                    f'RBEActor "{self.user_info.name}" has '
